...
export default function SettingsPanel(){
  ...
  return (
    <div className="vstack gap-md" style={{marginTop:24}}>
      <h2>{t("settings")}</h2>
      ...
      <div className="vstack gap-sm">
        ...
        <div className="row">
          <button className={"btn "+(lang==="es"?"success":"")} onClick={()=>setLang("es")}>{t("spanish")}</button>
          <button className={"btn "+(lang==="en"?"success":"")} onClick={()=>setLang("en")}>{t("english")}</button>
        </div>
      </div>

      <div className="vstack gap-sm">
        <label className="title">{t("codeFontSize")} ({tempSize}px)</label>
        <input type="range" min="12" max="22" value={tempSize} onChange={e=>setTempSize(parseInt(e.target.value,10))}/>
        <button className="btn" onClick={()=>setCodeSize(tempSize)}>{t("save")}</button>
      </div>

      <div className="vstack gap-sm">
        <label className="checkbox">
          <input type="checkbox" checked={previewExpanded} onChange={e=>setPreviewExpanded(e.target.checked)} />
      ...
      <div className="vstack gap-sm">
        ...
        <div className="row">
          <button className={"btn "+(cardStyle==="bordered"?"success":"")} onClick={()=>setCardStyle("bordered")}>{t("bordered")}</button>
          <button className={"btn "+(cardStyle==="borderless"?"success":"")} onClick={()=>setCardStyle("borderless")}>{t("borderless")}</button>
          <button className={"btn "+(cardStyle==="shadow"?"success":"")} onClick={()=>setCardStyle("shadow")}>{t("shadow")}</button>
        </div>
      </div>
    </div>
  );
